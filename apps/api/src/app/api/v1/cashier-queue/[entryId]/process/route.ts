...
export const POST = withBusinessPermission(
  ...
  async (request, auth, routeContext) => {
    ...
    const body = (await request.json()) as { cashierEmployeeId?: string };

    if (!body.cashierEmployeeId) {
      return jsonError('VALIDATION_ERROR', 'cashierEmployeeId is required', requestId, {
        status: 400,
      });
    }

    const { cashierQueueService } = getBusinessContainer();
    const entry = await cashierQueueService.startProcessing(
      auth.tenantId,
      entryId,
